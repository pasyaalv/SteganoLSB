...
'''
    ...
    def main(self,root):
        ...
        title = Label(f,text='Pilih Option untuk Encode atau Decode Gambar:')
        ...
        ascii_art.config(font=('courier',16,'bold'))

        root.grid_rowconfigure(1, weight=1)
        root.grid_columnconfigure(0, weight=1)

        f.grid()
        ascii_art.grid(row=1,pady=50)
        title.grid(row=2)
        b_encode.grid(row=3)
        b_decode.grid(row=4)
    ...
    def frame1_decode(self,f):
        f.destroy()
        d_f2 = Frame(root)
        label_art = Label(d_f2, text=self.decodeArt)
        label_art.config(font=('courier',8))
    ...
    def frame2_decode(self,d_f2):
        ...
        if not myfile:
            ...
            l4= Label(d_f3,text='Gambar yang dipilih :')
            ...
            l2 = Label(d_f3, text='Hidden text:')
    ...
    def frame1_encode(self,f):
        f.destroy()
        f2 = Frame(root)
        label_art = Label(f2, text=self.encodeArt)
        label_art.config(font=('courier',8))
        label_art.grid(row =1,pady=50)
        l1= Label(f2,text='Pilih Gambar yang akan \ndisisipkan text :')
        l1.config(font=('courier',18))
        l1.grid()
    ...
    def frame2_encode(self,f2):
        ...
        if not myfile:
            ...
            myimage = myimg.resize((300,200))
            img = ImageTk.PhotoImage(myimage)
            l3= Label(ep,text='Gambar yang dipilih : ')
            ...
            panel.grid()
            l2 = Label(ep, text='Masukan hidden text :')
            l2.config(font=('courier',18))
            l2.grid(pady=15)
